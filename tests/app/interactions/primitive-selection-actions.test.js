...
import { isAdditiveSelectionEvent } from '../../../src/app/interactions/gesture-layer.js';
...
describe('createPrimitiveSelectionActions', () => {
  ...
  it('adds a clicked 2D primitive to the current selection when Shift is held', () => {
    const selectedAtomIds = new Set(['a1']);
    const selectedBondIds = new Set();
    const calls = [];
    const mol2d = {
    ...
    actions.handle2dPrimitiveClick(
      {
        shiftKey: true,
        preventDefault() {
          prevented = true;
