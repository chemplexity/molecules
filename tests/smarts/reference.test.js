...
describe('functionalGroups — hydrocarbons', () => {
  ...
  it('cyclopentadiene found in cyclopentadiene', () =>
    assert.equal(matchesSMARTS(mol('C1=CCC=C1'), fg.cyclopentadiene.smarts), true));
  it('cyclohexadiene found in 1,3-cyclohexadiene', () =>
    assert.equal(matchesSMARTS(mol('C1=CCCC=C1'), fg.cyclohexadiene.smarts), true));
  it('cycloheptatriene found in cycloheptatriene', () =>
    assert.equal(matchesSMARTS(mol('C1=CC=CCC=C1'), fg.cycloheptatriene.smarts), true));
...
describe('functionalGroups — oxygen groups', () => {
  ...
  it('aldehyde found in acetaldehyde', () =>
    assert.equal(matchesSMARTS(parseSMILES('CC=O'), fg.aldehyde.smarts), true));
  ...
  it('carboxylicAcid found in acetic acid', () =>
    assert.equal(matchesSMARTS(parseSMILES('CC(=O)O'), fg.carboxylicAcid.smarts), true));
  it('ester found in methyl acetate', () => assert.equal(matchesSMARTS(mol('CC(=O)OC'), fg.ester.smarts), true));
  it('ester NOT found in acetic acid', () =>
    assert.equal(matchesSMARTS(parseSMILES('CC(=O)O'), fg.ester.smarts), false));
  ...
  it('anhydride found in acetic anhydride', () =>
    assert.equal(matchesSMARTS(mol('CC(=O)OC(=O)C'), fg.anhydride.smarts), true));
  it('acylHalide found in acetyl chloride', () =>
    assert.equal(matchesSMARTS(mol('CC(=O)Cl'), fg.acylHalide.smarts), true));
  it('peroxide found in hydrogen peroxide', () =>
    assert.equal(matchesSMARTS(parseSMILES('OO'), fg.peroxide.smarts), true));
});
...
describe('functionalGroups — nitrogen groups', () => {
  it('primaryAmine found in methylamine', () =>
    assert.equal(matchesSMARTS(parseSMILES('CN'), fg.primaryAmine.smarts), true));
  it('secondaryAmine found in dimethylamine', () =>
    assert.equal(matchesSMARTS(parseSMILES('CNC'), fg.secondaryAmine.smarts), true));
  it('tertiaryAmine found in trimethylamine', () =>
    assert.equal(matchesSMARTS(mol('CN(C)C'), fg.tertiaryAmine.smarts), true));
  ...
  it('nitro found in nitrobenzene (neutral form)', () =>
    assert.equal(matchesSMARTS(mol('c1ccc([N+](=O)[O-])cc1'), fg.nitro.smarts), true));
  ...
  it('isocyanate found in methyl isocyanate', () =>
    assert.equal(matchesSMARTS(mol('CN=C=O'), fg.isocyanate.smarts), true));
...
describe('functionalGroups — sulfur groups', () => {
  ...
  it('disulfide found in dimethyl disulfide', () =>
    assert.equal(matchesSMARTS(mol('CSSC'), fg.disulfide.smarts), true));
  it('sulfone found in DMSO2', () => assert.equal(matchesSMARTS(mol('CS(=O)(=O)C'), fg.sulfone.smarts), true));
  it('sulfonamide found in methanesulfonamide', () =>
    assert.equal(matchesSMARTS(parseSMILES('CS(=O)(=O)N'), fg.sulfonamide.smarts), true));
});
...
describe('functionalGroups — halogens', () => {
  it('organofluoride found in fluoromethane', () =>
    assert.equal(matchesSMARTS(mol('CF'), fg.organofluoride.smarts), true));
  it('organochloride found in chloromethane', () =>
    assert.equal(matchesSMARTS(mol('CCl'), fg.organochloride.smarts), true));
  it('organobromide found in bromomethane', () =>
    assert.equal(matchesSMARTS(mol('CBr'), fg.organobromide.smarts), true));
  ...
  it('arylHalide found in chlorobenzene', () =>
    assert.equal(matchesSMARTS(mol('c1ccccc1Cl'), fg.arylHalide.smarts), true));
  it('arylHalide NOT found in chloromethane', () =>
    assert.equal(matchesSMARTS(mol('CCl'), fg.arylHalide.smarts), false));
...
describe('functionalGroups — aromatic heterocycles', () => {
  ...
  it('quinoline found in quinoline', () =>
    assert.equal(matchesSMARTS(mol('c1ccc2ncccc2c1'), fg.quinoline.smarts), true));
});
...
describe('functionalGroups — phosphorus groups', () => {
  it('phosphoricAcid found in phosphoric acid', () =>
    assert.equal(matchesSMARTS(mol('P(=O)(O)(O)O'), fg.phosphoricAcid.smarts), true));
  it('phosphoricAcid NOT found in trimethyl phosphate', () =>
    assert.equal(matchesSMARTS(mol('COP(=O)(OC)OC'), fg.phosphoricAcid.smarts), false));
  it('phosphate (triester) found in trimethyl phosphate', () =>
    assert.equal(matchesSMARTS(mol('COP(=O)(OC)OC'), fg.phosphate.smarts), true));
  it('phosphate (triester) NOT found in methyl dihydrogen phosphate', () =>
    assert.equal(matchesSMARTS(mol('COP(=O)(O)O'), fg.phosphate.smarts), false));
  it('phosphate (triester) NOT found in phosphoric acid', () =>
    assert.equal(matchesSMARTS(mol('P(=O)(O)(O)O'), fg.phosphate.smarts), false));
  it('phosphateDiester found in [PH](=O)(OC)(O)OC', () =>
    assert.equal(matchesSMARTS(mol('[PH](=O)(OC)(O)OC'), fg.phosphateDiester.smarts), true));
  it('phosphateDiester NOT found in trimethyl phosphate', () =>
    assert.equal(matchesSMARTS(mol('COP(=O)(OC)OC'), fg.phosphateDiester.smarts), false));
  it('phosphateMonoester found in methyl dihydrogen phosphate', () =>
    assert.equal(matchesSMARTS(mol('COP(=O)(O)O'), fg.phosphateMonoester.smarts), true));
  it('phosphateMonoester NOT found in trimethyl phosphate', () =>
    assert.equal(matchesSMARTS(mol('COP(=O)(OC)OC'), fg.phosphateMonoester.smarts), false));
  it('phosphonate found in dimethyl methylphosphonate', () =>
    assert.equal(matchesSMARTS(mol('CP(=O)(OC)OC'), fg.phosphonate.smarts), true));
  it('phosphonate NOT found in trimethyl phosphate (no P-C bond)', () =>
    assert.equal(matchesSMARTS(mol('COP(=O)(OC)OC'), fg.phosphonate.smarts), false));
  it('phosphonate NOT found in [PH] ester (no P-C bond)', () =>
    assert.equal(matchesSMARTS(mol('[PH](=O)(OC)(OC)OC'), fg.phosphonate.smarts), false));
  it('phosphine found in trimethylphosphine', () =>
    assert.equal(matchesSMARTS(mol('CP(C)C'), fg.phosphine.smarts), true));
  it('phosphineOxide found in trimethylphosphine oxide', () =>
    assert.equal(matchesSMARTS(mol('CP(=O)(C)C'), fg.phosphineOxide.smarts), true));
});
