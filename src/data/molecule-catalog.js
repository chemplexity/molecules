...
export function findMolecules(query, options = {}) {
  ...
  for (const collection of moleculeCatalog) {
    ...
    for (const molecule of collection.molecules) {
      const haystack = [molecule.id, molecule.name, molecule.smiles, molecule.inchi, ...(molecule.tags ?? []), ...(molecule.aliases ?? []), ...collectionFields].map(normalizeCollectionSearchValue);
