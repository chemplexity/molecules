 ...
 */
export const STANDARD_ROTATION_ANGLES = Object.freeze([0, Math.PI / 6, -Math.PI / 6, Math.PI / 3, -Math.PI / 3, Math.PI / 2, -Math.PI / 2, (2 * Math.PI) / 3, -(2 * Math.PI) / 3, Math.PI]);

/**
 * Coarse fallback rotation candidate set, dropping the ±30° entries.
 */
export const COARSE_ROTATION_ANGLES = Object.freeze([0, Math.PI / 3, -Math.PI / 3, Math.PI / 2, -Math.PI / 2, (2 * Math.PI) / 3, -(2 * Math.PI) / 3, Math.PI]);
