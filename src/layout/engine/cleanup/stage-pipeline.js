...
const HOOK_STEP_META = {
  ...
  'terminal-alkene-continuation-relief': [
    'Terminal Multiple-Bond Continuation Relief',
    'Terminal carbon multiple-bond tails rotated away from local overlaps while keeping alkene bends and alkyne continuations exact.'
  ],
...
export function buildCleanupStageGraph(context) {
  ...
  const specialistCleanupComparator = (candidate, incumbent) => {
    ...
    const ligandAnglesImprove = (candidate.ligandAngleDeviation ?? Number.POSITIVE_INFINITY) < (incumbent?.ligandAngleDeviation ?? Number.POSITIVE_INFINITY) - PRESENTATION_METRIC_EPSILON;
  ...
  const presentationCleanupComparator = (candidate, incumbent) => {
    if (auditFinalStereoWithTieBreak(candidate, incumbent)) {
      return true;
    }
    if (!candidate.audit || !incumbent?.audit || (incumbent.audit.ok === true && candidate.audit.ok !== true)) {
      return false;
    }
    if (
      (candidate.audit.bondLengthFailureCount ?? 0) > (incumbent.audit.bondLengthFailureCount ?? 0) ||
  ...
  const terminalAlkeneContinuationRetouchComparator = (candidate, incumbent) => {
    ...
    if (
      (candidate.terminalAlkeneContinuationMaxPenalty ?? Number.POSITIVE_INFINITY) > (incumbent?.terminalAlkeneContinuationMaxPenalty ?? Number.POSITIVE_INFINITY) + PRESENTATION_METRIC_EPSILON ||
      (candidate.terminalAlkeneContinuationPenalty ?? Number.POSITIVE_INFINITY) > (incumbent?.terminalAlkeneContinuationPenalty ?? Number.POSITIVE_INFINITY) + PRESENTATION_METRIC_EPSILON
    ) {
      return false;
    }
    return (
