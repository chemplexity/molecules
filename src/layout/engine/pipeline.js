...
const FINAL_BRIDGED_PAIRED_TERMINAL_BRANCH_MAX_HEAVY_ATOMS = 100;
const FINAL_BRIDGED_PAIRED_TERMINAL_BRANCH_MAX_SUBTREE_HEAVY_ATOMS = 3;
const FINAL_BRIDGED_PAIRED_TERMINAL_BRANCH_ROTATIONS = Object.freeze([15, 20, 25, 30, 45, 60, 90, 120, 150, 180].map(degrees => (degrees * Math.PI) / 180).flatMap(rotation => [rotation, -rotation]));
...
const FINAL_COMPACT_BRIDGED_OVERLAP_KK_HALOGEN_VERTEX_SHIFT_FACTORS = Object.freeze([0.08, 0.1, 0.12, 0.14, 0.16, 0.18]);
...
const FINAL_PAIRED_STRETCHED_BRIDGED_RING_BOND_MAX_HEAVY_ATOMS = 100;
...
const FINAL_ULTRA_LARGE_CLUSTERED_PEPTIDE_ROTATIONS = Object.freeze(Array.from({ length: 10 }, (_value, index) => index + 1).flatMap(step => [(step * Math.PI) / 90, (-step * Math.PI) / 90]));
...
/**
 * Returns whether a clean large-molecule geometry checkpoint should replace a
 * later final-retouch result that introduced a new audited defect.
 * @param {object|null} checkpointAudit - Audit captured for the clean checkpoint.
 * @param {object|null} finalAudit - Audit for the current final coordinates.
 * @returns {boolean} True when restoring the checkpoint is safer.
 */
export function shouldRestoreLargeMoleculeCleanCheckpoint(checkpointAudit, finalAudit) {
  return checkpointAudit?.ok === true && checkpointAudit?.fallback?.mode == null && !finalAuditCountsDoNotWorsen(finalAudit, checkpointAudit);
}
...
function maybeRetouchFinalBridgedRingSubstituentWithPathRelief(layoutGraph, finalCoords, placement, bondLength) {
  const baseAudit = auditLayout(layoutGraph, finalCoords, {
    bondLength,
    bondValidationClasses: placement.bondValidationClasses
  });
  if (
    ...
    (baseAudit.labelOverlapCount ?? 0) !== 0 ||
    (baseAudit.stereoContradiction ?? false) ||
    smallHeteroRingSubstituentLayoutHeavyAtomCount(layoutGraph, finalCoords) > 40
  ) {
    return { coords: finalCoords, changed: false, movedAtomIds: [], audit: baseAudit };
  }
  ...
  for (const anchorAtomId of finalCoords.keys()) {
    ...
    for (const { childAtomId: rootAtomId } of collectReadableRingSubstituentChildren(layoutGraph, finalCoords, anchorAtomId)) {
      const rootAtom = layoutGraph.atoms.get(rootAtomId);
      const anchorBond = layoutGraph.bondByAtomPair.get(atomPairKey(anchorAtomId, rootAtomId));
      if (!rootAtom || rootAtom.element !== 'O' || layoutGraph.ringAtomIdSet.has(rootAtomId) || !anchorBond || anchorBond.kind !== 'covalent' || anchorBond.inRing || (anchorBond.order ?? 1) !== 1) {
        continue;
      }
      const branchAtomIds = [...collectCutSubtree(layoutGraph, rootAtomId, anchorAtomId)].filter(atomId => finalCoords.has(atomId));
      const branchHeavyAtomIds = branchAtomIds.filter(atomId => layoutGraph.atoms.get(atomId)?.element !== 'H');
      if (branchHeavyAtomIds.length < 3 || branchHeavyAtomIds.length > 8 || branchHeavyAtomIds.some(atomId => layoutGraph.ringAtomIdSet.has(atomId))) {
        continue;
      }
      const pivot = finalCoords.get(anchorAtomId);
      if (!pivot) {
        continue;
      }
      for (let degrees = -178; degrees <= 178; degrees += 2) {
        if (degrees === 0) {
          continue;
        }
        const rotation = (degrees * Math.PI) / 180;
        const branchCoords = cloneCoords(finalCoords);
        for (const atomId of branchAtomIds) {
          branchCoords.set(atomId, rotateAround(finalCoords.get(atomId), pivot, rotation));
        }
        const branchAudit = auditLayout(layoutGraph, branchCoords, {
          bondLength,
          bondValidationClasses: placement.bondValidationClasses
        });
        if (
          (branchAudit.ringSubstituentReadabilityFailureCount ?? 0) !== 0 ||
          (branchAudit.severeOverlapCount ?? 0) !== 1 ||
          (branchAudit.bondLengthFailureCount ?? 0) !== 0 ||
          (branchAudit.labelOverlapCount ?? 0) !== 0 ||
          (branchAudit.stereoContradiction ?? false)
        ) {
          continue;
        }
        const overlap = findSevereOverlaps(layoutGraph, branchCoords, bondLength)[0];
        if (!overlap) {
          continue;
        }
        const branchAtomIdSet = new Set(branchAtomIds);
        const pathAtomId = branchAtomIdSet.has(overlap.firstAtomId) ? overlap.secondAtomId : branchAtomIdSet.has(overlap.secondAtomId) ? overlap.firstAtomId : null;
        const pathAtom = pathAtomId ? layoutGraph.atoms.get(pathAtomId) : null;
        if (!pathAtom || (pathAtom.heavyDegree ?? 0) !== 2 || (layoutGraph.ringCountByAtomId.get(pathAtomId) ?? 0) !== 1) {
          continue;
        }
        const endpointAtomIds = (layoutGraph.bondsByAtomId.get(pathAtomId) ?? []).flatMap(bond => {
          if (!bond || bond.kind !== 'covalent' || !bond.inRing) {
            return [];
          }
          const neighborAtomId = bond.a === pathAtomId ? bond.b : bond.a;
          return finalCoords.has(neighborAtomId) ? [neighborAtomId] : [];
        });
        if (endpointAtomIds.length !== 2) {
          continue;
        }
        if (layoutGraph.atomToRingSystemId.get(pathAtomId) !== ringSystemId) {
          continue;
        }
        const pathMovedAtomIds = finalExactBridgedRingPathMoveGroup(layoutGraph, branchCoords, pathAtomId, new Set(ringSystem.atomIds ?? []));
        const pathDescriptor = {
          atomId: pathAtomId,
          blockerAtomId: branchAtomIdSet.has(overlap.firstAtomId) ? overlap.firstAtomId : overlap.secondAtomId,
          endpointAtomIds,
          movedAtomIds: pathMovedAtomIds
        };
        const pathReliefCoords = singleBridgedRingPathAtomOverlapCandidates(branchCoords, pathDescriptor, bondLength);
        const firstEndpoint = branchCoords.get(endpointAtomIds[0]);
        const secondEndpoint = branchCoords.get(endpointAtomIds[1]);
        const pathPosition = branchCoords.get(pathAtomId);
        if (firstEndpoint && secondEndpoint && pathPosition) {
          const midpoint = {
            x: (firstEndpoint.x + secondEndpoint.x) / 2,
            y: (firstEndpoint.y + secondEndpoint.y) / 2
          };
          for (const radiusFactor of [1 / 3, 0.4, 0.5, 0.6, 0.75]) {
            for (let angleIndex = 0; angleIndex < 36; angleIndex++) {
              const angle = (angleIndex * Math.PI) / 18;
              const target = add(midpoint, fromAngle(angle, radiusFactor * bondLength));
              const candidateCoords = cloneCoords(branchCoords);
              translateAtomGroup(candidateCoords, branchCoords, pathMovedAtomIds, sub(target, pathPosition));
              pathReliefCoords.push(candidateCoords);
            }
          }
        }
        for (const candidateCoords of pathReliefCoords) {
          const candidateAudit = auditLayout(layoutGraph, candidateCoords, {
            bondLength,
            bondValidationClasses: placement.bondValidationClasses
          });
          ...
          const movedAtomIds = [...new Set([...branchAtomIds, ...pathMovedAtomIds])];
          const candidate = {
            coords: candidateCoords,
            changed: true,
            audit: candidateAudit,
          ...
          if (!bestCandidate || candidate.totalMove < bestCandidate.totalMove - PRESENTATION_METRIC_EPSILON) {
            bestCandidate = candidate;
          }
        }
      }
  ...
  return bestCandidate ?? { coords: finalCoords, changed: false, movedAtomIds: [], audit: baseAudit };
}
...
function maybeRepairCompactBridgedOverlapKamadaKawaiHalogenVertex(molecule, layoutGraph, coords, placement, bondLength, bondValidationClasses, audit) {
  if ((audit.severeOverlapCount ?? 0) !== 1 || (audit.bondLengthFailureCount ?? 0) !== 0) {
    return null;
  }
  ...
  for (const [movedAtomId, blockerAtomId] of [
    ...
    if (movedAtom?.element !== 'C' || blockerAtom?.element !== 'C' || !layoutGraph.ringAtomIdSet.has(movedAtomId)) {
      continue;
    }
    const terminalHalogenIds = [...(layoutGraph.bondsByAtomId.get(movedAtomId) ?? [])]
      .map(bond => (bond.a === movedAtomId ? bond.b : bond.a))
      .filter(atomId => {
        const atom = layoutGraph.atoms.get(atomId);
        return atom && ['F', 'Cl', 'Br', 'I'].includes(atom.element) && atom.heavyDegree === 1 && coords.has(atomId);
      });
    if (terminalHalogenIds.length === 0) {
      continue;
    }
    const movedPosition = coords.get(movedAtomId);
    ...
    for (const shiftFactor of FINAL_COMPACT_BRIDGED_OVERLAP_KK_HALOGEN_VERTEX_SHIFT_FACTORS) {
      const candidateCoords = cloneCoords(coords);
      const displacement = {
        x: (dx / distance) * bondLength * shiftFactor,
        y: (dy / distance) * bondLength * shiftFactor
      };
      for (const atomId of [movedAtomId, ...terminalHalogenIds]) {
        const position = candidateCoords.get(atomId);
        candidateCoords.set(atomId, { x: position.x + displacement.x, y: position.y + displacement.y });
      }
    ...
    for (const secondMode of ['first', 'second', 'both']) {
      for (const firstShiftFactor of FINAL_COMPACT_BRIDGED_OVERLAP_KK_PAIRED_CLOSURE_SHIFT_FACTORS) {
        for (const secondShiftFactor of FINAL_COMPACT_BRIDGED_OVERLAP_KK_PAIRED_CLOSURE_SHIFT_FACTORS) {
          const candidateCoords = compactBridgedOverlapKamadaKawaiClosureCandidate(coords, closures, [firstMode, secondMode], [bondLength * firstShiftFactor, bondLength * secondShiftFactor]);
          if (!candidateCoords) {
            continue;
          }
          const candidateAudit = auditFinalRetouchCoords(molecule, layoutGraph, candidateCoords, placement, bondLength, bondValidationClasses);
...
function maybeSeedFinalCompactBridgedOverlapKamadaKawai(molecule, layoutGraph, finalCoords, placement, bondLength) {
  ...
  if (
    ...
    (currentAudit.severeOverlapCount ?? 0) > FINAL_COMPACT_BRIDGED_OVERLAP_KK_SEED_MAX_OVERLAPS ||
    (currentAudit.bondLengthFailureCount ?? 0) !== 0 ||
    (currentAudit.labelOverlapCount ?? 0) > FINAL_COMPACT_BRIDGED_OVERLAP_KK_SEED_MAX_LABEL_OVERLAPS ||
    (currentAudit.ringSubstituentReadabilityFailureCount ?? 0) !== 0 ||
    (currentAudit.collapsedMacrocycleCount ?? 0) !== 0 ||
    (currentAudit.stereoContradiction ?? false)
  ) {
  ...
  const kkResult = layoutKamadaKawai(layoutGraph.sourceMolecule ?? molecule, heavyAtomIds, {
    bondLength,
    threshold: FINAL_COMPACT_BRIDGED_WHOLE_COMPONENT_KK_THRESHOLD,
    innerThreshold: FINAL_COMPACT_BRIDGED_WHOLE_COMPONENT_KK_THRESHOLD,
    maxIterations: FINAL_COMPACT_BRIDGED_WHOLE_COMPONENT_KK_MAX_ITERATIONS,
    maxInnerIterations: FINAL_COMPACT_BRIDGED_WHOLE_COMPONENT_KK_MAX_INNER_ITERATIONS
  });
  if (kkResult.coords.size !== heavyAtomIds.length) {
  ...
  if ((candidateAudit.severeOverlapCount ?? 0) === 1 && (candidateAudit.bondLengthFailureCount ?? 0) === 0) {
    const repairedCandidate = maybeRepairCompactBridgedOverlapKamadaKawaiHalogenVertex(molecule, layoutGraph, candidateCoords, placement, bondLength, candidateBondValidationClasses, candidateAudit);
    if (repairedCandidate) {
      candidateCoords = repairedCandidate.coords;
  ...
  if ((candidateAudit.bondLengthFailureCount ?? 0) === 2) {
    const repairedCandidate = maybeRepairCompactBridgedOverlapKamadaKawaiClosures(molecule, layoutGraph, candidateCoords, placement, bondLength, candidateBondValidationClasses, candidateAudit);
    if (!repairedCandidate) {
      return unchanged();
    }
...
function stretchedBridgedRingBondCompactNeighborTailGroups(layoutGraph, coords, startAtomId, blockedAtomId) {
  ...
  for (const bond of layoutGraph.bondsByAtomId.get(startAtomId) ?? []) {
    ...
    const neighborAtomId = bond.a === startAtomId ? bond.b : bond.a;
    const neighborAtom = layoutGraph.atoms.get(neighborAtomId);
    if (neighborAtomId === blockedAtomId || !neighborAtom || neighborAtom.element === 'H' || neighborAtom.chirality != null || (neighborAtom.heavyDegree ?? 0) > 3) {
      continue;
    }
...
function stretchedBridgedRingBondDescriptors(layoutGraph, coords, placement, bondLength, baseAudit) {
  if (
    ...
    (baseAudit.collapsedMacrocycleCount ?? 0) !== 0 ||
    (baseAudit.stereoContradiction ?? false) ||
    (baseAudit.visibleHeavyBondCrossingCount ?? 0) > 4 ||
...
function organometallicMildBridgedRingBondNeighborAtomIds(layoutGraph, coords, atomId, blockedAtomId) {
  const atomIds = [];
  for (const bond of layoutGraph.bondsByAtomId.get(atomId) ?? []) {
    const neighborAtomId = bond.a === atomId ? bond.b : bond.a;
    const neighborAtom = layoutGraph.atoms.get(neighborAtomId);
    if (
      ...
      isMetalAtom(layoutGraph.sourceMolecule?.atoms.get(neighborAtomId) ?? neighborAtom) ||
      !coords.has(neighborAtomId) ||
      layoutGraph.fixedCoords?.has(neighborAtomId)
    ) {
      continue;
    }
    atomIds.push(...organometallicMildBridgedRingBondEndpointAtomIds(layoutGraph, coords, neighborAtomId));
  }
  return [...new Set(atomIds)];
}
...
function organometallicMildBridgedRingBondDescriptors(layoutGraph, coords, placement, bondLength, baseAudit, familySummary) {
  ...
  for (const bond of layoutGraph.bonds.values()) {
    ...
    descriptors.push({
      ...
      firstNeighborAtomIds: organometallicMildBridgedRingBondNeighborAtomIds(layoutGraph, coords, bond.a, bond.b),
      secondNeighborAtomIds: organometallicMildBridgedRingBondNeighborAtomIds(layoutGraph, coords, bond.b, bond.a),
      requiredShift
    });
  }
  return descriptors;
...
function organometallicMildBridgedRingBondCandidate(coords, descriptor, shiftFactor, firstNeighborWeight = 0, secondNeighborWeight = 0) {
  const firstPosition = coords.get(descriptor.firstAtomId);
  const secondPosition = coords.get(descriptor.secondAtomId);
  if (!firstPosition || !secondPosition) {
    return null;
  ...
  if (firstNeighborWeight > 0) {
    translateAtomGroup(candidateCoords, coords, descriptor.firstNeighborAtomIds, {
      x: unit.x * shift * 0.5 * firstNeighborWeight,
      y: unit.y * shift * 0.5 * firstNeighborWeight
    });
  }
  if (secondNeighborWeight > 0) {
    translateAtomGroup(candidateCoords, coords, descriptor.secondNeighborAtomIds, {
      x: -unit.x * shift * 0.5 * secondNeighborWeight,
      y: -unit.y * shift * 0.5 * secondNeighborWeight
    });
  }
  return {
    coords: candidateCoords,
    movedAtomIds: [
      ...new Set([
        ...descriptor.firstMovedAtomIds,
        ...descriptor.secondMovedAtomIds,
        ...(firstNeighborWeight > 0 ? descriptor.firstNeighborAtomIds : []),
        ...(secondNeighborWeight > 0 ? descriptor.secondNeighborAtomIds : [])
      ])
    ]
  };
}
...
function maybeRetouchFinalOrganometallicMildBridgedRingBond(molecule, layoutGraph, finalCoords, placement, bondLength, familySummary) {
  ...
  for (const descriptor of descriptors) {
    for (const shiftFactor of FINAL_STRETCHED_BRIDGED_RING_BOND_SHIFT_FACTORS) {
      for (const firstNeighborWeight of [0, 0.25, 0.5, 0.75, 1]) {
        for (const secondNeighborWeight of [0, 0.25, 0.5, 0.75, 1]) {
          const candidate = organometallicMildBridgedRingBondCandidate(finalCoords, descriptor, shiftFactor, firstNeighborWeight, secondNeighborWeight);
          if (!candidate) {
            continue;
          }
          const candidateAudit = auditFinalRetouchCoords(molecule, layoutGraph, candidate.coords, placement, bondLength);
...
function countLargeMoleculeAmideCenters(layoutGraph) {
  let count = 0;
  for (const atom of layoutGraph.atoms.values()) {
    if (atom.element !== 'C') {
      continue;
    }
    let hasCarbonylOxygen = false;
    ...
    for (const bond of layoutGraph.bondsByAtomId.get(atom.id) ?? []) {
      ...
      const neighbor = layoutGraph.atoms.get(neighborAtomId);
      if (neighbor?.element === 'O' && (bond.order ?? 1) === 2) {
        hasCarbonylOxygen = true;
      }
      if (neighbor?.element === 'N' && (bond.order ?? 1) === 1) {
        hasAmideNitrogen = true;
      }
    }
    if (hasCarbonylOxygen && hasAmideNitrogen) {
      count++;
    }
  }
  return count;
...
function finalPeptideRoutingCandidateIsBetter(candidateAudit, candidateMovedHeavyAtomCount, bestCandidate) {
  if (!bestCandidate) {
    return true;
  }
  for (const key of ['severeOverlapCount', 'visibleHeavyBondCrossingFailureCount', 'visibleHeavyBondCrossingCount']) {
    const candidateValue = candidateAudit[key] ?? 0;
    const bestValue = bestCandidate.audit[key] ?? 0;
    if (candidateValue !== bestValue) {
      return candidateValue < bestValue;
    }
  }
  const candidatePenalty = candidateAudit.severeOverlapPenalty ?? 0;
  const bestPenalty = bestCandidate.audit.severeOverlapPenalty ?? 0;
  if (Math.abs(candidatePenalty - bestPenalty) > PRESENTATION_METRIC_EPSILON) {
    return candidatePenalty < bestPenalty;
  }
  return candidateMovedHeavyAtomCount < bestCandidate.movedHeavyAtomCount;
}

function finalPeptideRoutingFocusBondIds(layoutGraph, audit) {
  const focusAtomIds = new Set();
  for (const overlap of audit.severeOverlaps ?? []) {
    focusAtomIds.add(overlap.firstAtomId);
    focusAtomIds.add(overlap.secondAtomId);
  }
  for (const crossing of audit.visibleHeavyBondCrossings ?? []) {
  ...
  let frontier = [...focusAtomIds];
  const visitedAtomIds = new Set(frontier);
...
function rotatedFinalPeptideRoutingCoords(coords, descriptor, rotation) {
  const pivot = coords.get(descriptor.anchorAtomId);
  if (!pivot) {
    return null;
  }
  const candidateCoords = cloneCoords(coords);
...
function maybeResolveFinalPeptidePairedCrossings(layoutGraph, coords, placement, bondLength, currentAudit) {
  ...
  const defectClusters =
    layoutGraph.traits.heavyAtomCount >= 400
      ? finalPeptideRoutingDefectClusters(severeOverlaps, visibleHeavyBondCrossings).filter(cluster => cluster.visibleHeavyBondCrossings.length > 0)
  ...
  const candidateIsBetter = candidateAudit => {
    ...
    const candidateResidualCount = (candidateAudit.severeOverlapCount ?? 0) + (candidateAudit.visibleHeavyBondCrossingFailureCount ?? 0);
    const bestResidualCount = (bestCandidate.audit.severeOverlapCount ?? 0) + (bestCandidate.audit.visibleHeavyBondCrossingFailureCount ?? 0);
      ...
      for (const firstRotation of FINAL_PEPTIDE_PAIRED_CROSSING_ROTATIONS) {
        ...
        for (const secondDescriptor of descriptors) {
          ...
          for (const secondRotation of FINAL_PEPTIDE_PAIRED_CROSSING_ROTATIONS) {
            if (probes >= FINAL_PEPTIDE_PAIRED_CROSSING_MAX_PROBES || (layoutGraph.traits.heavyAtomCount >= 400 && clusterProbes >= FINAL_PEPTIDE_PAIRED_CROSSING_MAX_CLUSTER_PROBES)) {
              break;
            }
            probes++;
            clusterProbes++;
            ...
            if ((candidateAudit.visibleHeavyBondCrossingFailureCount ?? 0) === 0 && (candidateAudit.severeOverlapCount ?? 0) === 0) {
              break pairedSearch;
            }
          }
        }
  ...
  return bestCandidate
    ...
    : { changed: false, coords, movedAtomIds: [], audit: currentAudit, probes };
}

/**
 * Unfolds collision-local peptide suffixes through small rigid rotations around
 * acyclic single bonds. Only audit-preserving moves are accepted.
 * @param {object} layoutGraph - Layout graph shell.
 * @param {Map<string, {x: number, y: number}>} inputCoords - Current coordinates.
 * @param {object} placement - Placement result containing frozen atoms and validation classes.
 * @param {number} bondLength - Target depiction bond length.
 * @returns {{changed: boolean, coords: Map<string, {x: number, y: number}>, movedAtomIds: string[], passes: number, audit: object|null}} Routing result.
 */
function maybeRouteFinalLargeMoleculePeptideBranches(layoutGraph, inputCoords, placement, bondLength) {
  if (countLargeMoleculeAmideCenters(layoutGraph) < FINAL_PEPTIDE_BRANCH_ROUTING_MIN_AMIDE_COUNT) {
    return { changed: false, coords: inputCoords, movedAtomIds: [], passes: 0, audit: null };
  }
  let coords = inputCoords;
  let currentAudit = auditLayout(layoutGraph, coords, {
    bondLength,
    bondValidationClasses: placement.bondValidationClasses
  });
  ...
  const movedAtomIds = new Set();
  let passes = 0;
  const maxPasses = layoutGraph.traits.heavyAtomCount >= 400 ? FINAL_ULTRA_LARGE_PEPTIDE_BRANCH_ROUTING_MAX_PASSES : FINAL_PEPTIDE_BRANCH_ROUTING_MAX_PASSES;
  while (passes < maxPasses && (currentAudit.severeOverlapCount ?? 0) > 0) {
    ...
    for (const bondId of focusBondIds) {
      ...
      for (const [anchorAtomId, rootAtomId] of [
        ...
        const pivot = coords.get(anchorAtomId);
        if (!pivot) {
          continue;
        }
        for (const rotation of FINAL_PEPTIDE_BRANCH_ROUTING_ROTATIONS) {
          const candidateCoords = cloneCoords(coords);
          for (const atomId of subtreeAtomIds) {
            candidateCoords.set(atomId, rotateAround(coords.get(atomId), pivot, rotation));
          }
          ...
          const overlapImproved =
            ...
            ((candidateAudit.severeOverlapCount ?? 0) === (currentAudit.severeOverlapCount ?? 0) &&
              (candidateAudit.severeOverlapPenalty ?? 0) + PRESENTATION_METRIC_EPSILON < (currentAudit.severeOverlapPenalty ?? 0));
          if (!overlapImproved || !finalAuditCountsDoNotWorsen(candidateAudit, currentAudit)) {
            continue;
          }
          if (finalPeptideRoutingCandidateIsBetter(candidateAudit, movedHeavyAtomCount, bestCandidate)) {
            bestCandidate = {
              coords: candidateCoords,
              audit: candidateAudit,
...
function maybeRelieveFinalLargeMoleculeAngles(molecule, layoutGraph, finalCoords, placement, bondLength) {
  ...
  let changed = false;
  const heavyAtomCount = layoutGraph.traits?.heavyAtomCount ?? layoutGraph.atoms?.size ?? 0;
  let remainingCandidateAudits = heavyAtomCount <= FINAL_LARGE_MOLECULE_ANGLE_RELIEF_UNBOUNDED_MAX_HEAVY_ATOMS ? Number.POSITIVE_INFINITY : FINAL_LARGE_MOLECULE_ANGLE_RELIEF_MAX_CANDIDATE_AUDITS;
...
function compactBridgedPeripheralRingPathShiftDescriptors(layoutGraph, coords, bondLength) {
  ...
  for (const ring of layoutGraph.rings ?? []) {
    ...
    const ringSystem = ringSystemId != null ? layoutGraph.ringSystemById.get(ringSystemId) : null;
    const ringIds = new Set(ringSystem?.ringIds ?? []);
    const hasBridgedRingPair = (layoutGraph.ringConnections ?? []).some(connection => connection.kind === 'bridged' && ringIds.has(connection.firstRingId) && ringIds.has(connection.secondRingId));
...
function finalBridgedPairedTerminalBranchDescriptors(layoutGraph, coords, placement, focusAtomIds, bondLength) {
  const descriptors = [];
  const seen = new Set();
  for (const bond of layoutGraph.bonds.values()) {
    if (!bond || bond.kind !== 'covalent' || bond.inRing || bond.aromatic || (bond.order ?? 1) !== 1) {
    ...
    for (const [anchorAtomId, rootAtomId] of [
      ...
      const subtreeHeavyAtomIds = subtreeAtomIds.filter(atomId => layoutGraph.atoms.get(atomId)?.element !== 'H');
      if (
        subtreeHeavyAtomIds.length === 0 ||
        subtreeHeavyAtomIds.length > FINAL_BRIDGED_PAIRED_TERMINAL_BRANCH_MAX_SUBTREE_HEAVY_ATOMS ||
        subtreeHeavyAtomIds.some(atomId => layoutGraph.ringAtomIdSet.has(atomId)) ||
        subtreeAtomIds.some(atomId => placement.frozenAtomIds?.has(atomId))
      ) {
        continue;
      }
      const nearFocus = subtreeHeavyAtomIds.some(atomId => {
        const position = coords.get(atomId);
        return [...focusAtomIds].some(focusAtomId => {
          const focusPosition = coords.get(focusAtomId);
          return position && focusPosition && Math.hypot(position.x - focusPosition.x, position.y - focusPosition.y) < bondLength * 1.5;
        });
      });
      if (!nearFocus) {
        continue;
...
function rotateFinalBridgedTerminalBranch(coords, descriptor, rotation) {
  const pivot = coords.get(descriptor.anchorAtomId);
  if (!pivot) {
    return null;
  }
...
function maybeRetouchFinalBridgedPairedTerminalBranchContacts(layoutGraph, finalCoords, placement, bondLength) {
  const baseAudit = auditLayout(layoutGraph, finalCoords, {
    bondLength,
    bondValidationClasses: placement.bondValidationClasses
  });
  const heavyAtomCount = layoutGraph.traits?.heavyAtomCount ?? [...layoutGraph.atoms.values()].filter(atom => atom.element !== 'H').length;
  if (
    heavyAtomCount > FINAL_BRIDGED_PAIRED_TERMINAL_BRANCH_MAX_HEAVY_ATOMS ||
    (layoutGraph.traits?.bridgedRingConnectionCount ?? 0) <= 0 ||
    (baseAudit.severeOverlapCount ?? 0) !== 1 ||
    (baseAudit.bondLengthFailureCount ?? 0) !== 0 ||
    (baseAudit.visibleHeavyBondCrossingFailureCount ?? 0) !== 0 ||
    (baseAudit.labelOverlapCount ?? 0) !== 0
  ) {
    return { changed: false, coords: finalCoords, movedAtomIds: [], audit: baseAudit };
  }
  const overlap = findSevereOverlaps(layoutGraph, finalCoords, bondLength)[0];
  if (!overlap) {
    return { changed: false, coords: finalCoords, movedAtomIds: [], audit: baseAudit };
  }
  const overlapAtomIds = new Set([overlap.firstAtomId, overlap.secondAtomId]);
  const descriptors = finalBridgedPairedTerminalBranchDescriptors(layoutGraph, finalCoords, placement, overlapAtomIds, bondLength);
  const primaryDescriptors = descriptors.filter(descriptor => descriptor.subtreeHeavyAtomIds.some(atomId => overlapAtomIds.has(atomId)));
  let bestCandidate = null;
  for (const primaryDescriptor of primaryDescriptors) {
    for (const secondaryDescriptor of descriptors) {
      if (
        secondaryDescriptor === primaryDescriptor ||
        secondaryDescriptor.subtreeAtomIds.some(atomId => primaryDescriptor.subtreeAtomIdSet.has(atomId)) ||
        primaryDescriptor.subtreeAtomIds.includes(secondaryDescriptor.anchorAtomId) ||
        secondaryDescriptor.subtreeAtomIds.includes(primaryDescriptor.anchorAtomId)
      ) {
        continue;
      }
      for (const primaryRotation of FINAL_BRIDGED_PAIRED_TERMINAL_BRANCH_ROTATIONS) {
        const primaryCoords = rotateFinalBridgedTerminalBranch(finalCoords, primaryDescriptor, primaryRotation);
        if (!primaryCoords) {
          continue;
        }
        for (const secondaryRotation of FINAL_BRIDGED_PAIRED_TERMINAL_BRANCH_ROTATIONS) {
          const candidateCoords = rotateFinalBridgedTerminalBranch(primaryCoords, secondaryDescriptor, secondaryRotation);
          if (!candidateCoords) {
            continue;
          }
          const candidateAudit = auditLayout(layoutGraph, candidateCoords, {
          ...
          const movedAtomIds = [...new Set([...primaryDescriptor.subtreeAtomIds, ...secondaryDescriptor.subtreeAtomIds])];
          const candidate = {
            coords: candidateCoords,
            audit: candidateAudit,
            movedAtomIds,
            rotationMagnitude: Math.abs(primaryRotation) + Math.abs(secondaryRotation),
            totalMove: finalAcyclicBranchContactCandidateMove(finalCoords, candidateCoords, movedAtomIds)
          };
          if (
            !bestCandidate ||
            candidate.rotationMagnitude < bestCandidate.rotationMagnitude - PRESENTATION_METRIC_EPSILON ||
            (Math.abs(candidate.rotationMagnitude - bestCandidate.rotationMagnitude) <= PRESENTATION_METRIC_EPSILON && candidate.totalMove < bestCandidate.totalMove)
          ) {
            bestCandidate = candidate;
          }
        }
...
function maybeRetouchFinalUltraLargeClusteredPeptideOverlaps(layoutGraph, finalCoords, placement, bondLength) {
  ...
  for (const rootAtomId of candidateRootAtomIds) {
    ...
    for (const bond of layoutGraph.bondsByAtomId.get(rootAtomId) ?? []) {
      ...
      if (subtreeHeavyAtomCount < FINAL_ULTRA_LARGE_CLUSTERED_PEPTIDE_MIN_SUBTREE_HEAVY_ATOMS || subtreeHeavyAtomCount > FINAL_ULTRA_LARGE_CLUSTERED_PEPTIDE_MAX_SUBTREE_HEAVY_ATOMS) {
        continue;
      }
  ...
  for (const descriptor of descriptors) {
    for (const rotation of FINAL_ULTRA_LARGE_CLUSTERED_PEPTIDE_ROTATIONS) {
      ...
      const visibleHeavyBondCrossingCount = countVisibleHeavyBondCrossingsAfterFocusedMove(layoutGraph, finalCoords, coords, descriptor.subtreeAtomIds, baseAudit.visibleHeavyBondCrossingCount ?? 0);
  ...
  for (let firstIndex = 0; firstIndex < shortlistedCandidates.length; firstIndex++) {
    ...
    for (let secondIndex = 0; secondIndex < shortlistedCandidates.length; secondIndex++) {
      ...
      const approximateCrossingCount = countVisibleHeavyBondCrossingsAfterFocusedMove(layoutGraph, finalCoords, candidateCoords, movedAtomIds, baseAudit.visibleHeavyBondCrossingCount ?? 0);
      if (
...
export function runPipeline(molecule, options = {}) {
  ...
  const preserveLargeMoleculeCleanGeometry = familySummary.primaryFamily === 'large-molecule' || placement.placedFamilies.includes('large-molecule');
  let largeMoleculeCleanCheckpoint = null;
  const captureLargeMoleculeCleanCheckpoint = stage => {
    if (!preserveLargeMoleculeCleanGeometry) {
      return;
    }
    const audit = auditLayout(layoutGraph, finalCoords, {
      bondLength: normalizedOptions.bondLength,
      bondValidationClasses: placement.bondValidationClasses
    });
    if (audit.ok === true && audit.fallback?.mode == null) {
      largeMoleculeCleanCheckpoint = {
        stage,
        coords: cloneCoords(finalCoords),
        audit
      };
    }
  };
  ...
  const applyFinalBridgedSingleOverlapRelaxation = () => {
    ...
    const pairedTerminalBranchRetouch = timeFinalRetouch('finalBridgedPairedTerminalBranchRetouch', () =>
      maybeRetouchFinalBridgedPairedTerminalBranchContacts(layoutGraph, finalCoords, placement, normalizedOptions.bondLength)
    );
    if (pairedTerminalBranchRetouch.changed) {
      const currentAudit = auditLayout(layoutGraph, finalCoords, {
        bondLength: normalizedOptions.bondLength,
        bondValidationClasses: placement.bondValidationClasses
      });
      finalCoords = pairedTerminalBranchRetouch.coords;
      finalCoordsModified = true;
      onStep?.('Final Bridged Paired Branch Retouch', 'Two short bridged-cage substituents rotated together to clear a trapped contact.', cloneCoords(finalCoords), {
        movedAtomCount: pairedTerminalBranchRetouch.movedAtomIds.length,
        severeOverlapCountBefore: currentAudit.severeOverlapCount,
        severeOverlapCountAfter: pairedTerminalBranchRetouch.audit?.severeOverlapCount ?? null,
        visibleHeavyBondCrossingCountBefore: currentAudit.visibleHeavyBondCrossingCount,
        visibleHeavyBondCrossingCountAfter: pairedTerminalBranchRetouch.audit?.visibleHeavyBondCrossingCount ?? null
      });
      return pairedTerminalBranchRetouch;
    }
  ...
  if (cleanup.largeDirtyFallbackFastPath === true) {
    ...
    if (
      ...
      if (largeDirtyPeptideBranchRouting.changed) {
        ...
        cleanup.finalStageAudit = null;
        cleanup.finalStageStereo = null;
        onStep?.('Large Dirty Peptide Branch Routing', 'Collision-local amide-rich suffixes were opened before returning from the ultra-large fast path.', cloneCoords(finalCoords), {
      ...
      const postRoutingLargeDirtyResidualRetouch = timeFinalRetouch('postRoutingLargeDirtyResidualRetouch', () =>
        runLargeMoleculeResidualRetouch(layoutGraph, finalCoords, {
          bondLength: normalizedOptions.bondLength,
          residualOnly: true
        })
      );
      if (postRoutingLargeDirtyResidualRetouch.changed) {
        finalCoords = postRoutingLargeDirtyResidualRetouch.coords;
        finalCoordsModified = true;
        cleanup.finalStageAudit = null;
        cleanup.finalStageStereo = null;
        onStep?.('Post-Routing Large Dirty Residual Retouch', 'Residual contacts and crossings were reduced after peptide branch routing settled.', cloneCoords(finalCoords), {
          movedAtomCount: postRoutingLargeDirtyResidualRetouch.movedAtomIds.length,
          severeOverlapCountBefore: postRoutingLargeDirtyResidualRetouch.severeOverlapCountBefore,
  ...
  captureLargeMoleculeCleanCheckpoint('pre-final-retouch');
  ...
  if (familySummary.primaryFamily !== 'acyclic') {
    ...
    captureLargeMoleculeCleanCheckpoint('large-molecule-residual-retouch');
  }
  ...
  if (shouldRunFinalLargeMoleculeAngleRelief(layoutGraph, familySummary)) {
    ...
    captureLargeMoleculeCleanCheckpoint('large-molecule-presentation-retouch');
  }
  ...
  if (familySummary.mixedMode === true) {
    timeFinalRetouch('mixedFinalBranchRetouches', () => {
      ...
      if (
        ...
        const bridgedRingExitRetouch = maybeRetouchFinalBridgedRingSubstituentWithPathRelief(layoutGraph, finalCoords, placement, normalizedOptions.bondLength);
        if (bridgedRingExitRetouch.changed && finalAuditCountsDoNotWorsen(bridgedRingExitRetouch.audit, postAcylAudit)) {
          const currentAudit = postAcylAudit;
          finalCoords = bridgedRingExitRetouch.coords;
          finalCoordsModified = true;
          postAcylAudit = bridgedRingExitRetouch.audit;
          onStep?.('Final Bridged Ring Exit Path Relief', 'Acyclic ring substituent rotated outward together with a bounded neighboring cage-path re-arc.', cloneCoords(finalCoords), {
            movedAtomCount: bridgedRingExitRetouch.movedAtomIds.length,
            ringSubstituentReadabilityFailureCountBefore: currentAudit.ringSubstituentReadabilityFailureCount,
            ringSubstituentReadabilityFailureCountAfter: postAcylAudit.ringSubstituentReadabilityFailureCount,
            severeOverlapCountBefore: currentAudit.severeOverlapCount,
            severeOverlapCountAfter: postAcylAudit.severeOverlapCount
  ...
  const finalBridgedPairedTerminalBranchRetouch =
    familySummary.primaryFamily === 'bridged'
      ? timeFinalRetouch('finalBridgedPairedTerminalBranchRetouch', () => maybeRetouchFinalBridgedPairedTerminalBranchContacts(layoutGraph, finalCoords, placement, normalizedOptions.bondLength))
      : { changed: false };
  if (finalBridgedPairedTerminalBranchRetouch.changed) {
    const currentAudit = auditLayout(layoutGraph, finalCoords, {
      bondLength: normalizedOptions.bondLength,
      bondValidationClasses: placement.bondValidationClasses
    });
    finalCoords = finalBridgedPairedTerminalBranchRetouch.coords;
    finalCoordsModified = true;
    onStep?.('Final Bridged Paired Branch Retouch', 'Two short bridged-cage substituents rotated together to clear a trapped contact.', cloneCoords(finalCoords), {
      movedAtomCount: finalBridgedPairedTerminalBranchRetouch.movedAtomIds.length,
      severeOverlapCountBefore: currentAudit.severeOverlapCount,
      severeOverlapCountAfter: finalBridgedPairedTerminalBranchRetouch.audit?.severeOverlapCount ?? null,
      visibleHeavyBondCrossingCountBefore: currentAudit.visibleHeavyBondCrossingCount,
      visibleHeavyBondCrossingCountAfter: finalBridgedPairedTerminalBranchRetouch.audit?.visibleHeavyBondCrossingCount ?? null
    });
  }
  const finalBridgedSingleOverlapRelaxation = finalBridgedPairedTerminalBranchRetouch.changed
    ? { changed: false }
    : timeFinalRetouch('finalBridgedSingleOverlapRelaxation', () => maybeRelaxFinalBridgedSingleOverlaps(layoutGraph, finalCoords, placement, normalizedOptions.bondLength));
  if (finalBridgedSingleOverlapRelaxation.changed) {
    const currentAudit = auditLayout(layoutGraph, finalCoords, {
      bondLength: normalizedOptions.bondLength,
      bondValidationClasses: placement.bondValidationClasses
  ...
  const finalCompactBridgedOverlapKamadaKawaiSeed =
    familySummary.primaryFamily === 'bridged'
      ? timeFinalRetouch('finalCompactBridgedOverlapKamadaKawaiSeed', () =>
          maybeSeedFinalCompactBridgedOverlapKamadaKawai(workingMolecule, layoutGraph, finalCoords, placement, normalizedOptions.bondLength)
        )
      : { changed: false };
  if (finalCompactBridgedOverlapKamadaKawaiSeed.changed) {
    finalCoords = finalCompactBridgedOverlapKamadaKawaiSeed.coords;
    placement.bondValidationClasses = finalCompactBridgedOverlapKamadaKawaiSeed.bondValidationClasses;
    finalCoordsModified = true;
    onStep?.('Final Compact Bridged Overlap KK Seed', 'A compact bridged cage was re-seeded to clear its trapped contact before final closure repair.', cloneCoords(finalCoords), {
      movedAtomCount: finalCompactBridgedOverlapKamadaKawaiSeed.movedAtomIds.length,
      severeOverlapCountAfter: finalCompactBridgedOverlapKamadaKawaiSeed.audit?.severeOverlapCount ?? null,
      bondLengthFailureCountAfter: finalCompactBridgedOverlapKamadaKawaiSeed.audit?.bondLengthFailureCount ?? null
    });
  }
  ...
  if (familySummary.primaryFamily === 'large-molecule' || placement.placedFamilies.includes('large-molecule')) {
    ...
    if ((lateLargeMoleculeResidualAudit.severeOverlapCount ?? 0) > 0 || (lateLargeMoleculeResidualAudit.visibleHeavyBondCrossingFailureCount ?? 0) > 0) {
      ...
      if (lateLargeMoleculeResidualRetouch.changed) {
        ...
        const overlapImproved = (candidateAudit.severeOverlapCount ?? 0) < (lateLargeMoleculeResidualAudit.severeOverlapCount ?? 0);
        const crossingImproved =
          (candidateAudit.severeOverlapCount ?? 0) <= (lateLargeMoleculeResidualAudit.severeOverlapCount ?? 0) &&
          (candidateAudit.visibleHeavyBondCrossingFailureCount ?? 0) < (lateLargeMoleculeResidualAudit.visibleHeavyBondCrossingFailureCount ?? 0);
        if (
  ...
  if (preserveLargeMoleculeCleanGeometry) {
    const finalPeptideBranchRouting = timeFinalRetouch('finalPeptideBranchRouting', () =>
      maybeRouteFinalLargeMoleculePeptideBranches(layoutGraph, finalCoords, placement, normalizedOptions.bondLength)
    );
    if (finalPeptideBranchRouting.changed) {
      const auditBefore = auditLayout(layoutGraph, finalCoords, {
        bondLength: normalizedOptions.bondLength,
        bondValidationClasses: placement.bondValidationClasses
      });
      finalCoords = finalPeptideBranchRouting.coords;
      finalCoordsModified = true;
      onStep?.('Final Peptide Branch Routing', 'Folded amide-repeat suffixes were opened through small rigid rotations around collision-local peptide bonds.', cloneCoords(finalCoords), {
        passes: finalPeptideBranchRouting.passes,
        pairedCrossingProbes: finalPeptideBranchRouting.pairedCrossingProbes ?? 0,
        movedAtomCount: finalPeptideBranchRouting.movedAtomIds.length,
        severeOverlapCountBefore: auditBefore.severeOverlapCount,
        severeOverlapCountAfter: finalPeptideBranchRouting.audit?.severeOverlapCount ?? null,
        visibleHeavyBondCrossingCountBefore: auditBefore.visibleHeavyBondCrossingCount,
        visibleHeavyBondCrossingCountAfter: finalPeptideBranchRouting.audit?.visibleHeavyBondCrossingCount ?? null
      });
    }
  }
  ...
  if (familySummary.primaryFamily === 'large-molecule' || placement.placedFamilies.includes('large-molecule')) {
    const finalLargeMoleculeResidualAudit = auditLayout(layoutGraph, finalCoords, {
      bondLength: normalizedOptions.bondLength,
      bondValidationClasses: placement.bondValidationClasses
    });
    if ((finalLargeMoleculeResidualAudit.severeOverlapCount ?? 0) > 0 || (finalLargeMoleculeResidualAudit.visibleHeavyBondCrossingFailureCount ?? 0) > 0) {
      const finalLargeMoleculeResidualRetouch = timeFinalRetouch('finalLargeMoleculeResidualRetouch', () =>
        runLargeMoleculeResidualRetouch(layoutGraph, finalCoords, {
          bondLength: normalizedOptions.bondLength,
          residualOnly: true
        })
      );
      if (finalLargeMoleculeResidualRetouch.changed) {
        const candidateAudit = auditLayout(layoutGraph, finalLargeMoleculeResidualRetouch.coords, {
          bondLength: normalizedOptions.bondLength,
          bondValidationClasses: placement.bondValidationClasses
        });
        const overlapImproved = (candidateAudit.severeOverlapCount ?? 0) < (finalLargeMoleculeResidualAudit.severeOverlapCount ?? 0);
        const crossingImproved =
          (candidateAudit.severeOverlapCount ?? 0) <= (finalLargeMoleculeResidualAudit.severeOverlapCount ?? 0) &&
          (candidateAudit.visibleHeavyBondCrossingFailureCount ?? 0) < (finalLargeMoleculeResidualAudit.visibleHeavyBondCrossingFailureCount ?? 0);
        if ((overlapImproved || crossingImproved) && finalAuditCountsDoNotWorsen(candidateAudit, finalLargeMoleculeResidualAudit)) {
          finalCoords = finalLargeMoleculeResidualRetouch.coords;
          finalCoordsModified = true;
          onStep?.('Final Large Molecule Residual Retouch', 'Residual contacts and crossings were repaired after all late presentation stages settled.', cloneCoords(finalCoords), {
            movedAtomCount: finalLargeMoleculeResidualRetouch.movedAtomIds.length,
            severeOverlapCountBefore: finalLargeMoleculeResidualAudit.severeOverlapCount,
            severeOverlapCountAfter: candidateAudit.severeOverlapCount,
            visibleHeavyBondCrossingCountBefore: finalLargeMoleculeResidualAudit.visibleHeavyBondCrossingCount,
            visibleHeavyBondCrossingCountAfter: candidateAudit.visibleHeavyBondCrossingCount
          });
        }
      }
  ...
  if (largeMoleculeCleanCheckpoint) {
    ...
    if (shouldRestoreLargeMoleculeCleanCheckpoint(largeMoleculeCleanCheckpoint.audit, finalGeometryAudit)) {
      finalCoords = cloneCoords(largeMoleculeCleanCheckpoint.coords);
      finalCoordsModified = true;
      onStep?.(
        'Large Molecule Clean Checkpoint Restore',
        'A clean large-molecule geometry checkpoint was restored after later presentation retouches introduced a new audited defect.',
        cloneCoords(finalCoords),
        {
          checkpointStage: largeMoleculeCleanCheckpoint.stage,
          severeOverlapCountBefore: finalGeometryAudit.severeOverlapCount,
          severeOverlapCountAfter: largeMoleculeCleanCheckpoint.audit.severeOverlapCount,
          visibleHeavyBondCrossingCountBefore: finalGeometryAudit.visibleHeavyBondCrossingCount,
          visibleHeavyBondCrossingCountAfter: largeMoleculeCleanCheckpoint.audit.visibleHeavyBondCrossingCount,
          bondLengthFailureCountBefore: finalGeometryAudit.bondLengthFailureCount,
          bondLengthFailureCountAfter: largeMoleculeCleanCheckpoint.audit.bondLengthFailureCount
        }
      );
    }
  }
