...
function minimumPreferredDeviation(candidateAngle, preferredAngles) {
  let preferredDeviation = Infinity;
  for (const preferredAngle of preferredAngles ?? []) {
    const deviation = angularDifference(candidateAngle, preferredAngle);
    if (deviation < preferredDeviation) {
      preferredDeviation = deviation;
    }
  }
  return preferredDeviation;
}
...
function buildNearPreferredRescueCandidateAngles(preferredAngles = []) {
  const rescueStep = DEG15 / 3;
  const rescueOffsets = [];
  for (let offset = rescueStep; offset <= DEG60 + 1e-9; offset += rescueStep) {
 ...
 * unlike ordinary flexible alkyl chains.
 * @param {object|null} layoutGraph - Layout graph shell.
 * @param {string} anchorAtomId - Aromatic heteroatom anchor ID.
 * @param {string} childAtomId - Candidate carbon substituent root ID.
 * @param {object} anchorBond - Bond between anchor and child.
 * @returns {boolean} True when the root should preserve exact ring-outward placement.
 */
function isHeteroarylConjugatedMethyleneRoot(layoutGraph, anchorAtomId, childAtomId, anchorBond) {
  ...
  for (const childBond of layoutGraph.bondsByAtomId.get(childAtomId) ?? []) {
    if (!childBond || childBond === anchorBond || childBond.kind !== 'covalent') {
      continue;
    }
    if (childBond.aromatic || (childBond.order ?? 1) !== 1) {
      return false;
    }
...
export function isExactRingOutwardEligibleSubstituent(layoutGraph, anchorAtomId, childAtomId) {
  ...
  if (isHeteroarylConjugatedMethyleneRoot(layoutGraph, anchorAtomId, childAtomId, bond)) {
    return true;
  }
  for (const childBond of layoutGraph.bondsByAtomId.get(childAtomId) ?? []) {
 ...
 * @param {string} anchorAtomId - Anchor atom ID.
 * @param {string|null} childAtomId - Candidate child atom ID.
 * @returns {boolean} True when the child should use the exact trigonal bisector.
 */
...
function ringSystemHasOnlyIsolatedOrFusedConnections(layoutGraph, anchorAtomId) {
  const ringSystemId = layoutGraph?.atomToRingSystemId?.get(anchorAtomId);
...
function pickClosestPreferredCandidateAngle(candidates, bondLength, preferredAngles, clearanceContext = null) {
  if (candidates.length === 0) {
    return null;
  }
  if (clearanceContext) {
  ...
  let bestPreferredDeviation = minimumPreferredDeviation(bestCandidate.angle, preferredAngles);
  for (let index = 1; index < candidates.length; index++) {
    const candidate = candidates[index];
    const preferredDeviation = minimumPreferredDeviation(candidate.angle, preferredAngles);
    if (preferredDeviation < bestPreferredDeviation - 1e-9) {
      bestCandidate = candidate;
      bestPreferredDeviation = preferredDeviation;
      continue;
    }
    if (Math.abs(preferredDeviation - bestPreferredDeviation) > 1e-9) {
      continue;
    }
    if (candidate.clearanceScore > bestCandidate.clearanceScore) {
      bestCandidate = candidate;
      continue;
    }
    if (
 ...
 * @param {{sumX: number, sumY: number, count: number}|null} placementState - Running placement state.
 ...
 * @returns {number|null} Closest safe near-preferred angle, or null when none is available.
 */
export function chooseNearPreferredRescueAngle(anchorPosition, bondLength, coords, occupiedAngles, preferredAngles, excludedAtomIds, placementState, ringPolygons = [], atomGrid = null) {
  const candidateAngles = buildNearPreferredRescueCandidateAngles(preferredAngles);
  if (candidateAngles.length === 0) {
    return null;
  }
  const rescueCandidates = evaluateAngleCandidates(
    candidateAngles,
    occupiedAngles,
    preferredAngles,
    anchorPosition,
 ...
 * @param {object|null} [atomGrid] - Optional spatial grid used for clearance checks.
 * @param {boolean} [preferFinePreferredAnglesBeforeFallback] - Whether safe fine offsets around preferred angles should beat generic fallbacks.
...
export function chooseContinuationAngle(
  ...
  atomGrid = null,
  preferFinePreferredAnglesBeforeFallback = false
) {
  ...
  if (allowFinePreferredAngles && preferFinePreferredAnglesBeforeFallback && preferredCandidateAngles.length > 0) {
    const finePreferredCandidates = evaluateAngleCandidates(
