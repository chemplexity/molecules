...
import {
  ...
  FORCE_LAYOUT_INITIAL_SETTLE_TICKS,
  FORCE_LAYOUT_INITIAL_SETTLE_ALPHA,
  FORCE_LAYOUT_INITIAL_RESTART_ALPHA,
  FORCE_LAYOUT_EDIT_RESTART_ALPHA,
...
const forceHelpers = createForceHelpers({
  ...
  plotEl,
  simulation,
  viewportFitPadding: (pad, options = {}) => _viewportFitPadding(pad, options),
...
const forceSceneRenderer = createForceSceneRenderer(
  createForceSceneRendererDeps({
    ...
    forceLayoutDefaultZoomMultiplier: FORCE_LAYOUT_DEFAULT_ZOOM_MULTIPLIER,
    forceLayoutInitialKeepInViewTicks: FORCE_LAYOUT_INITIAL_KEEP_IN_VIEW_TICKS,
    forceLayoutInitialSettleTicks: FORCE_LAYOUT_INITIAL_SETTLE_TICKS,
    forceLayoutInitialSettleAlpha: FORCE_LAYOUT_INITIAL_SETTLE_ALPHA,
    forceLayoutInitialRestartAlpha: FORCE_LAYOUT_INITIAL_RESTART_ALPHA,
    forceLayoutEditRestartAlpha: FORCE_LAYOUT_EDIT_RESTART_ALPHA,
    ...
    reseatForceGraphHydrogens: (graph, options = {}) => forceHelpers.reseatForceGraphHydrogens(graph, options),
...
const { navigationActions, selectionActions, clipboardActions, editingActions, dragGestureActions, drawBondPreviewActions, drawBondCommitActions, primitiveSelectionActions, primitiveEventHandlers } =
  initializeInteractionRuntime(
    createInteractionRuntimeDeps({
      ...
      getPeriodicTableButton: () => domElements.getPeriodicTableButtonElement(),
      getPeriodicTablePopover: () => domElements.getPeriodicTablePopoverElement(),
      getPeriodicTableGrid: () => domElements.getPeriodicTableGridElement(),
      ...
      getElementButton: element => domElements.getElementButtonElements(element),
      ...
      viewportRestoreEditPolicy: ViewportPolicy.restoreEdit,
      clearStereoAnnotations: (mol, affectedIds) => mol.clearStereoAnnotations(affectedIds),
      kekulize,
      refreshAromaticity,
      patchNodePositions: (patchPos, options = {}) => forceHelpers.patchForceNodePositions(patchPos, options),
      ...
      renderMol: (mol, options = {}) => renderRuntime.renderMol(mol, options),
...
const { inputFlowManager, inputControls } = initializeAppRuntime(
  createAppRuntimeDeps({
    ...
    hasActiveResonanceView,
    getActiveResonanceSourceMolecule,
    paintReactionPreviewReactantSource: payload => _paintReactionPreviewReactantSource(payload),
