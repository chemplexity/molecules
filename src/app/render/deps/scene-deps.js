...
export function createForceSceneRendererDeps(ctx) {
  return {
    ...
    constants: {
      ...
      forceLayoutInitialRestartAlpha: ctx.forceLayoutInitialRestartAlpha,
      forceLayoutEditRestartAlpha: ctx.forceLayoutEditRestartAlpha,
    ...
    helpers: {
      ...
      reseatForceGraphHydrogens: (graph, options = {}) => ctx.reseatForceGraphHydrogens(graph, options),
      ...
      forceHydrogenPlacement: links => ctx.forceHydrogenPlacement(links),
...
export function create2DSceneRendererDeps(ctx) {
  return {
    ...
    helpers: {
      ...
      centerReaction2dPairCoords: (mol, spacing, options) => ctx.centerReaction2dPairCoords(mol, spacing, options),
      reactionArrowLabelMinGapBondLength: (mol, spacing) => ctx.reactionArrowLabelMinGapBondLength?.(mol, spacing),
